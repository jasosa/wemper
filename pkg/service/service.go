...
func (svc *Service) Server(port string) *http.Server {
	...
	router.Handle("/persons", LoggingMiddleware(svc.logger, getAllUsersHandler)).Methods("GET")

	invitePersonsHandler := ErrorHandler{HandleWithErrorFunc(svc.invitePersonHandler), svc.logger}
	router.Handle("/persons/{id}/invitations/", LoggingMiddleware(svc.logger, invitePersonsHandler)).Methods("POST")
